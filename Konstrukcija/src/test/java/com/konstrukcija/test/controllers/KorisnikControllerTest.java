...
public class KorisnikControllerTest {
		...
		mockMvc.perform(get(URL_PREFIX + "/all"))
				...
				.andExpect(
						jsonPath("$.[*].email").value(
								hasItem("nole0223@gmail.com")));
	}

	/**
	 * Testiranje metode koja vraca sve korisnike koji su aktivni to znaci da su
	 * verifikovali svoj nalog ili ga nisu izbrisali.
	 * 
	 * @throws Exception
	 */
	@Test
	@Transactional
	@Rollback(true)
	public void testGetAllActiveKorisnici() throws Exception {
		mockMvc.perform(get(URL_PREFIX + "/all/users"))
				.andExpect(status().isOk())
				.andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$", hasSize(1)))
				.andExpect(jsonPath("$.[*].fname").value(hasItem("Novica")))
	...
	/**
	 * Testiranje dva korisnika koja imaju isti email, test ne bi trebao da
	 * prodje. zato sto je zabranjeno da korisnici imaju isti email
	 * 
	 * @throws Exception
	 * 
	 * @author Novica Nikolic
	...
	public void testSaveKorisnik() throws Exception {
		...
		mockMvc.perform(
				post(URL_PREFIX + "/registration/korisnik").contentType(
						contentType).content(json)).andExpect(
				status().isCreated());
	...
	/**
	 * Testiranje metode za logovanje korisnika, prvo se logujemo sa ispravnim
	 * podacima posle se logijemo sa neisptavnim podacima
	 */
	@Test
	@Transactional
	@Rollback(true)
	public void testLogin() throws Exception {
		...
		mockMvc.perform(
				post(URL_PREFIX + "/login").contentType(contentType).content(
						json)).andExpect(status().isOk());
		...
		mockMvc.perform(
				post(URL_PREFIX + "/login").contentType(contentType).content(
						json)).andExpect(status().isNotFound());
	}

	/**
	 * Testiranje metode koja proverava verifikacioni kod kako bi bilo zavrseno
	...
	public void TestVerify() throws Exception {
		...
		mockMvc.perform(get(URL_PREFIX + "/verify/" + korisnik.getVerifyCode()))
				.andExpect(status().isOk());

		mockMvc.perform(
				get(URL_PREFIX + "/verify/" + korisnik.getVerifyCode()
						+ "asssss")).andExpect(status().isNotFound());
	}
